(() => {
  ...
  // src/annotator.ts
  var TEXT_EXCLUDED_TAGS = new Set([
    "SCRIPT",
    "STYLE",
    "NOSCRIPT",
    "TEMPLATE",
  ...
  function getAnnotatableElementText(element) {
    if (shouldExcludeElementText(element))
      return "";
    if (!(element instanceof HTMLElement))
      return "";
    return element.innerText.replace(/\s+/g, " ").trim();
  }
  function shouldExcludeElementText(element) {
  ...
  class Annotator {
    ...
    handleClick(e) {
      ...
      if (!target)
        return;
      if (target.closest("[data-annotate-toolbar]") || target.closest("[data-annotate-orb]") || target.closest("[data-annotate-popup]") || target.closest("[data-annotate-badge]")) {
        return;
      }
      e.preventDefault();
      e.stopPropagation();
    ...
    showHighlight(element) {
      this.removeHighlight();
      if (element.closest("[data-annotate-toolbar]") || element.closest("[data-annotate-orb]") || element.closest("[data-annotate-popup]") || element.closest("[data-annotate-badge]")) {
        return;
      }
    ...
    getSelector(element) {
      ...
      while (current && current !== document.body) {
        ...
        if (current.classList.length > 0) {
          const classes = Array.from(current.classList).filter((c) => !c.startsWith("ac-") && !c.startsWith("data-annotate")).slice(0, 2).map((c) => `.${c}`).join("");
          if (classes)
            selector += classes;
        }
        parts.unshift(selector);
  ...
  async function captureElement(element) {
    try {
      const canvas = await import_html2canvas.default(element, {
  ...
  // src/popup.ts
  var ICON_SLIDERS = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><line x1="4" y1="6" x2="14" y2="6"/><line x1="18" y1="6" x2="20" y2="6"/><circle cx="16" cy="6" r="2"/><line x1="4" y1="18" x2="6" y2="18"/><line x1="10" y1="18" x2="20" y2="18"/><circle cx="8" cy="18" r="2"/><line x1="4" y1="12" x2="10" y2="12"/><line x1="14" y1="12" x2="20" y2="12"/><circle cx="12" cy="12" r="2"/></svg>`;
  var ICON_MIC = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3a3 3 0 0 0-3 3v6a3 3 0 0 0 6 0V6a3 3 0 0 0-3-3Z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><path d="M12 19v3"/></svg>`;
  function showAnnotationPopup(options) {
  ...
  `;
    ...
    if (top + popupHeight > vh - 8)
      top = vh - popupHeight - 8;
    if (top < 8)
      top = 8;
    popup.style.left = `${left}px`;
    popup.style.top = `${top}px`;
    const textarea = popup.querySelector(".ac-popup-textarea");
    textarea.focus();
...
`;
  ...
  // src/toolbar.ts
  var ICON_CROSSHAIR = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#a1a1aa" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>`;
  var ICON_CROSSHAIR_ON = `<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#3b82f6" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>`;
  var ICON_PENCIL = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></svg>`;
  var ICON_X = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>`;
  var ICON_SEND_ALL = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>`;
  var ICON_BULLSEYE = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="6"/><circle cx="12" cy="12" r="2"/></svg>`;
  var ICON_LIST = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="8" y1="6" x2="21" y2="6"/><line x1="8" y1="12" x2="21" y2="12"/><line x1="8" y1="18" x2="21" y2="18"/><line x1="3" y1="6" x2="3.01" y2="6"/><line x1="3" y1="12" x2="3.01" y2="12"/><line x1="3" y1="18" x2="3.01" y2="18"/></svg>`;
  var ICON_TRASH = `<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg>`;
  function createToolbar(options) {
    ...
    `;
      ...
      function showTooltip() {
        ...
        if (tLeft + tooltipW > window.innerWidth - 8)
          tLeft = window.innerWidth - tooltipW - 8;
        if (tLeft < 8)
          tLeft = 8;
  ...
  class BadgeManager {
    badges = new Map;
    addBadge(targetElement, id, index, onClick) {
  ...
  var annotationCacheKey = "";
  var pendingAnnotationCacheKey = "";
  var modeCacheKey = "";
  var pendingMessageId = null;
  var pendingSendTimer = null;
  var ANNOTATION_CACHE_PREFIX = "opencode-annotate:annotations:";
  var PENDING_ANNOTATION_CACHE_PREFIX = "opencode-annotate:pending-annotations:";
  var MODE_CACHE_PREFIX = "opencode-annotate:mode:";
  ...
  async function handleElementSelect(element) {
    if (!wsClient || !enabled)
      return;
  ...
  function addQueueItem(element, screenshot, annotation) {
    ...
    if (badgeManager) {
      item.badge = badgeManager.addBadge(element.element, id, queue.length - 1, () => handleEditItem(item));
    }
    updateToolbar();
  ...
  function restoreCachedAnnotations() {
    if (!annotationCacheKey)
      return;
    let cached = [];
    try {
  ...
  function saveCachedAnnotations() {
    if (!annotationCacheKey)
      return;
    if (queue.length === 0) {
      clearCachedAnnotations();
  ...
  function clearCachedAnnotations() {
    if (!annotationCacheKey)
      return;
    try {
  ...
  function savePendingAnnotations(messageId, items) {
    if (!pendingAnnotationCacheKey)
      return;
    try {
      sessionStorage.setItem(pendingAnnotationCacheKey, JSON.stringify({
        messageId,
  ...
  function restorePendingAnnotations() {
    if (!pendingAnnotationCacheKey)
      return;
    try {
  ...
  function isCachedAnnotation(value) {
    if (!value || typeof value !== "object")
      return false;
    const item = value;
    return typeof item.id === "string" && typeof item.selector === "string" && typeof item.text === "string" && typeof item.annotation === "string" && (typeof item.screenshot === "string" || item.screenshot === null);
  }
  ...
  function restoreCachedItems(items) {
    for (const item of items) {
      ...
      if (badgeManager) {
        queueItem.badge = badgeManager.addBadge(element, queueItem.id, queue.length - 1, () => handleEditItem(queueItem));
      }
    }
  }
  function sendAnnotation(element, screenshot, annotation) {
    if (!wsClient) {
  ...
  function sendAnnotationBatch(messageId, items) {
    if (!wsClient) {
      console.error("[annotate] Cannot send: no wsClient");
      return false;
    }
    ...
    if (sent)
      console.log("[annotate] Batch message sent");
    return sent;
  }
  function updateToolbar() {
    toolbar?.updateQueue(queue.map((item) => ({
      selector: item.element.selector,
      annotation: item.annotation
    })));
  }
